  ...
  return (
    <>
      <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 lg:bottom-8 lg:right-8 z-50 flex flex-col items-center space-y-4">
        <img src={namaste} className="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 rounded-full shadow-lg" alt="Chatbot Icon" />
        <button
          type="button"
        ...
        >
          ...
          <span className='block text-xs md:text-xs lg:text-sm font-bold' style={{ fontFamily: "'Merriweather', serif" }}>Chat with Janaki</span>
        </button>
      </div>

      {isChatbotVisible && (
        <div style={{ fontFamily: "'Merriweather', serif" }} className="fixed bottom-4 right-4 md:bottom-6 md:right-6 lg:bottom-8 lg:right-8 z-50 w-[60%] max-w-sm md:max-w-md lg:max-w-lg h-80 md:h-96 lg:h-[500px] bg-white border-2 border-gray-300 rounded-lg shadow-xl flex flex-col">
          <div className="flex justify-between items-center bg-blue-500 p-3 rounded-t-lg h-16 shadow-md">
            <h1 className="text-lg md:text-xl lg:text-2xl font-bold text-white">CHAT WITH JANAKI</h1>
            <button
              type="button"
            ...
            <div aria-live="polite">
              {messages.map((message, index) => (
                <div key={index} className={`flex ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`p-3 my-2 rounded-lg shadow-md ${message.sender === 'user' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-black'} max-w-[80%] md:max-w-[70%] lg:max-w-[60%] relative text-xs md:text-sm lg:text-base`}>
                    {message.text}
          ...
          <div className="p-3 bg-gray-100 rounded-b-lg flex items-center">
            <input
              type="text"
              className="flex-1 p-2 border-2 border-gray-300 rounded-l-lg text-sm md:text-base focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={input}
            ...
            <button
              type="button"
              className="ml-2 bg-gradient-to-r from-blue-500 to-blue-700 text-white p-2 rounded-r-lg shadow-lg transition-transform transform hover:scale-105 active:bg-blue-800"
              onClick={handleSendMessage}
            >
              <span className="font-bold text-xs md:text-sm lg:text-base">Send</span>
            </button>
          </div>
        </div>
      )}
